...
def convert(inputs, activation, kernel_initializer):
    #32*32*3
...
#activation eg.tf.nn.sigmod, tf.nn.relu
#kernel_initializer eg.tf.glorot_uniform_initializer, tf.truncated_normal_initializer,tf.keras.initializers.he_normal
flatten = convert(x_image, tf.nn.relu,tf.truncated_normal_initializer)
...
#梯度下降的方法 eg. tf.train.GradientDescentOptimizer(1e-4).minmize(loss)
...
1, 指定显示变量
2, 输出到指定文件夹
3, cmd执行命令 tensorboard --logdir eg. tensorboard --logdir train:'train',test:'test'
"""
